...
static int __init ylcdrza1h_init_early(void)
{
	printk("=== %s ===\n",__func__);
 ...
 */
early_initcall(ylcdrza1h_init_early);
...
static int heartbeat(void * data)
{
	...
	while(1) {
		...
		if (index & 1)  {
			/* LED off */
			r7s72100_pfc_pin_assign(P11_11, PMODE, PORT_OUT_LOW);
		}
		else {
			/* LED on */
			r7s72100_pfc_pin_assign(P11_11, PMODE, PORT_OUT_HIGH);
		}
...
static int __init ylcdrza1h_init_late(void)
{
 ...
 */
late_initcall(ylcdrza1h_init_late);
