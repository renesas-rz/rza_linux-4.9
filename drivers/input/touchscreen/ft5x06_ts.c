 ...
 *
 * Copyright (C) 2014,2017 Renesas Electronics America.
 *
...
/* Default Resolution */
/* Can be set by Device Tree */
#define DEFAULT_X 1280
...
static int screenres[2] = {DEFAULT_X, DEFAULT_Y}; /* x, y */
...
#else
			if (ts->prop.invert_x) {
				p[i].x = ts->prop.max_x - p[i].x;
			}
			if (ts->prop.invert_x) {
				p[i].y = ts->prop.max_y - p[i].y;
			}
#endif
...
static int ft5x06_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts = devm_kzalloc(&client->dev, sizeof(struct ft5x06_ts), GFP_KERNEL);
	if (!ts) {
...
#else
	touchscreen_parse_properties(idev, true, &ts->prop);
#endif

	/* get idev */
	input_set_drvdata(idev, ts);

	/* Controller chip interrupt active low enable */
