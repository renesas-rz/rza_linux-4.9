 ...
 *
 * Converted to Device Tree by Renesas Electronics in 2019.
 *
...
static void rz_can_tx_failure_cleanup(struct net_device *ndev)
{
	...
	for (i = 0; i < RZ_CAN_TX_ECHO_SKB_MAX; i++)
		can_free_echo_skb(ndev, i);	//asdf BUG: Can't call can_free_echo_skb() from an ISR
}
...
static int rz_can_probe(struct platform_device *pdev)
{
	...
	if (!np) {
		dev_err(&pdev->dev, "No Device Tree data provided!\n");
		goto fail;
	}
	...
	if (of_property_read_u32(np, "renesas,channel", &priv->m))
	{
		dev_err(&pdev->dev, "Error: 'renesas,channel' property not set in DT\n");
		err = -EINVAL;
		goto fail;
	}

	if (of_property_read_u32(np, "renesas,clock-select", &priv->clock_select))
	{
		dev_err(&pdev->dev, "Error: 'clock-select' property not set in DT\n");
		err = -EINVAL;
		goto fail;
	}


	sprintf(name,"intrcan%drec", priv->m);
	priv->rx_irq = of_irq_get_byname(np, name);
	sprintf(name,"intrcan%dtrx", priv->m);
	priv->tx_irq = of_irq_get_byname(np, name);
	sprintf(name,"intrcan%derr", priv->m);
	priv->err_irq_m = of_irq_get_byname(np, name);
	priv->err_irq_g = of_irq_get_byname(np, "intrcangerr");

	if (IS_ERR_VALUE(priv->rx_irq) || IS_ERR_VALUE(priv->tx_irq) ||
	    IS_ERR_VALUE(priv->err_irq_m) || IS_ERR_VALUE(priv->err_irq_g)) {
	...
	priv->base = devm_ioremap(&pdev->dev, mem->start, resource_size(mem));
	...
	if (priv->clock_select == CLKR_CLKC)
		priv->can.clock.freq = clk_get_rate(priv->clk) / 2;
	else
		if (of_property_read_u32(np, "renesas,clk-xincan-rate", &priv->can.clock.freq)) {
			dev_err(&pdev->dev, "Error: 'renesas,clk-xincan-rate' not set in DT\n");
			err = -EINVAL;
			goto fail;
		}
...
static const struct of_device_id of_rz_can_match[] = {
	{ .compatible = "renesas,rza1_can",},
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, of_rz_can_match);
...
static struct platform_driver rz_can_driver = {
	...
	.driver = {
		.name = DRV_NAME,
		.owner = THIS_MODULE,
		.of_match_table = of_rz_can_match,
	},
};
